...
async def give_filter(client, message):
    ...
    if settings["auto_filter"]:
        if message.chat.id == SUPPORT_GROUP:
            ...
            if files:
                btn = [[
                    InlineKeyboardButton("Here", url='https://t.me/filmyspot0')
                ]]
        ...
        if btn:
            ...
            k = await message.reply(f"🙅‍♂️ Oh no! It seems like I forgot to verify", reply_markup=InlineKeyboardMarkup(btn), protect_content=True)
            await asyncio.sleep(300)
            await k.delete()
            try:
                await message.delete()
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('« ʙᴀᴄᴋ', callback_data='start'),
            InlineKeyboardButton('☎️ ᴄᴏɴᴛᴀᴄᴛ', url='https://t.me/BotGeniusProbot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
